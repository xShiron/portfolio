...
class FrameLSTMClassifier(nn.Module):
    def __init__(self):
        super(FrameLSTMClassifier, self).__init__()

        self.backbone = timm.create_model("legacy_xception", pretrained=True, num_classes=0, global_pool='avg')

        # Bi-LSTM
        self.lstm = nn.LSTM(input_size=2048, hidden_size=512, num_layers=1, 
    ...
    def forward(self, x):
        ...
        # LSTM
        lstm_out, (hn, cn) = self.lstm(feats)  
...
def test(model, criterion, test_loader, device, tb_writer):
    ...
    tb_writer.add_figure("Testing Confusion Matrix", fig)
...
if __name__ == "__main__":
    ...
    model_used = FrameLSTMClassifier()

    for param in model_used.backbone.parameters():
        param.requires_grad = False

    # Unfreeze last 18 layers
    children = list(model_used.backbone.named_children())

    # Get only the modules (drop names)
    last_modules = children[-18:]

    for name, module in last_modules:
